...
public class GaussianAlgorithm implements Operation {
    public Matrix solve(Matrix initialMatrix) throws NoSolutionException, IndexOutOfRangeException {
        ...
        for (int mColumn = 0; mColumn < matrix.getHorizontalSize(); mColumn++)
        {
            ...
            if(tryGetNotUsedRow(matrix, mColumn, wasRowBeenUsed).getFirst()) continue;
            ...
            for (int row = 0; row < matrix.getVerticalSize(); row++)
            {
                ...
                freeMembers.setElement(freeMembers.getElement(row, 0) +
                        freeMembers.getElement(activeRow, 0) * coefficient, row, 0);
            }
        }
    ...
    private Matrix getValueUnknownVariables(Matrix matrix, Matrix freeMembers)
        ...
        while (rowCounter < matrix.getVerticalSize())
        {
            ...
            result.setElement(
                    freeMembers.getElement(row, 0) / matrix.getElement(row, column), column, 0);
            if (column < matrix.getHorizontalSize() - 1)
                column++;
        }
    ...
    public Optional<Matrix> apply(List<Matrix> matrices) throws IncorrectNumberOfElements {
        ...
        var solver = new GaussianAlgorithm();
        try {
            return Optional.of(solver.solve(matrices.get(0)));
        } catch (NoSolutionException | IndexOutOfRangeException e) {
